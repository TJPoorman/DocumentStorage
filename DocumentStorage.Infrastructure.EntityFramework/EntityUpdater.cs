...
/// <inheritdoc/>
/// <remarks>Provides functionality to update and manage entities in a given Entity Framework <typeparamref name="TContext"/> context.</remarks>
/// <typeparam name="TContext">The type of the DbContext used for database operations.</typeparam>
public class EntityUpdater<TContext> : IDsEntityUpdater
    where TContext : DbContext
{
    ...
    /// <summary>
    /// Gets or sets a value indicating whether automatic change detection is enabled for the context.
    /// </summary>
    protected virtual bool AutoDetectChangesEnabled { get => _context.ChangeTracker.AutoDetectChangesEnabled; set => _context.ChangeTracker.AutoDetectChangesEnabled = value; }

    /// <summary>
    /// Initializes a new instance of the <see cref="EntityUpdater{TContext}"/> class with the specified DbContext.
    /// </summary>
    /// <param name="dbContext">The DbContext to be used for entity updates.</param>
    public EntityUpdater(TContext dbContext) => _context = dbContext;
    ...
    /// <summary>
    /// Forces the context to detect changes to its entities.
    /// </summary>
    protected virtual void DetectChanges() => _context.ChangeTracker.DetectChanges();
    ...
    protected virtual ObjectActivator GetObjectActivator(Type type)
    {
        ...
        DynamicMethod dynamicMethod = new("CreateInstance", type, Type.EmptyTypes, true);
    ...
    /// <summary>
    /// Removes a record of the specified type from the DbContext.
    /// </summary>
    ...
    protected void Remove(Type recordType, IDsRecord record)
    {
        ...
        bool originalDetectChangesEnabled = AutoDetectChangesEnabled;
        ...
        {
            if (AutoDetectChangesEnabled != originalDetectChangesEnabled)
            {
                AutoDetectChangesEnabled = originalDetectChangesEnabled;
                DetectChanges();
            }
        }
    }
    ...
    protected virtual bool ShouldRemove(Type recordType, IDsRecord record) => _context.ChangeTracker.Entries().Any(e => e.Entity.GetType() == recordType);
    ...
    protected void UpdateProperty(PropertyInfo sourceProperty, object sourceValue, PropertyInfo targetProperty, object target, object targetValue)
    {
        ...
        else if (typeof(IEnumerable).IsAssignableFrom(targetPropertyType) && targetPropertyType != typeof(string))
        {
            ...
            {
                Dictionary<Guid, IDsRecord> lookup = new();
